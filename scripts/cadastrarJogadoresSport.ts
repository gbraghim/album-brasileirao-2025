...
async function cadastrarJogadoresSport() {
  try {
    // Diretório onde estão as imagens dos jogadores
    const diretorioJogadores = path.join(process.cwd(), 'public', 'players', 'Sport');
    ...
    for (const imagem of imagens) {
      try {
        ...
        const jogadorExistente = await prisma.jogador.findFirst({
          where: {
            nome: nomeJogador,
            timeId: '18' // ID do Sport como string
          }
        });
